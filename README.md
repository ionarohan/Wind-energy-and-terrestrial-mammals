# Wind-energy-and-terrestrial-mammals

Data and code repository for "Terrestrial mammals alter habitat selection in response to turbine visibility at a central New Mexico wind energy facility" describing the effects of wind turbines and their associated access roads on the habitat selection of mule deer (Odocoileus hemionus), pronghorns (Antilocapra americana), coyotes (Canis latrans), gray foxes (Urocyon cinereoargenteus), kit foxes (Vulpes macrotis), American badgers (Taxidea taxus), striped skunks (Mephitis mephitis), black-tailed jackrabbits (Lepus californicus), and desert cottontails (Sylvilagus audubonii) from a remotely activated camera study.

Data Contributors: Iona M. Rohan, Theresa M. Laverty, and Jennifer K. Frey. 

# Folders

data - Contains raw data files for handling data

code - Contains R scripts for each species' base model and wind model as well as scripts for producing the associated plots

figures - Contains plots of the effects of variables on detection and habitat use from the base models and wind models

outputs - Contains tables produced from the base model and wind model code and found in Supporting Information S2 and S3
