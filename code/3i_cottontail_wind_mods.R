...
Habitat <- occu( ~  veg_cover_cam_under_1m + as.factor(biotic_com_2)
                 ~  NDVI_0_1km, occu.syau)

Null <- occu( ~  veg_cover_cam_under_1m + as.factor(biotic_com_2)  
              ~   1, occu.syau)

Bio_Com_Occu <- occu( ~  veg_cover_cam_under_1m + 
                         as.factor(biotic_com_2)        
                      ~  as.factor(biotic_com_2), occu.syau)

Canopy_Occu <- occu( ~  veg_cover_cam_under_1m + 
                        as.factor(biotic_com_2)        
                     ~  canopy_cov, occu.syau)
...
## The table with the comparison of the relative weight of evidence between
  ...
  #variable on the probability of habitat selection (ψ) of desert cottontail is 
  #found in Table S3.21.
...
wind.hab.cor <- site.covs %>% 
  select( biotic_com_2, NDVI_0_1km, canopy_cov,
          turbine_interior, X50cm_turbine_vis, turbine_dist,
          turbine_rd_dist, turbine_rd_density_0_1km)
...
Turbine_Int_Occu <- occu(  ~  veg_cover_cam_under_1m + 
                             as.factor(biotic_com_2)        
                           ~  as.factor(turbine_interior), occu.syau)

Habitat_Add_Turbine_Int <- occu( ~  veg_cover_cam_under_1m + 
                                   as.factor(biotic_com_2)        
                                 ~  NDVI_0_1km + 
                                   as.factor(turbine_interior), occu.syau)

Bio_Com_X_Turbine_Int <- occu( ~  veg_cover_cam_under_1m + 
                                 as.factor(biotic_com_2)        
                               ~  as.factor(turbine_interior) * 
                                 as.factor(biotic_com_2), occu.syau)

NDVI_X_Turbine_Int <- occu( ~  veg_cover_cam_under_1m + 
                              as.factor(biotic_com_2)      
                            ~  as.factor(turbine_interior) * NDVI_0_1km,
                            occu.syau)
...
# 85% CI for NDVI interaction 
# Extract coefficients and VCOV matrix
coef_est <- coef(NDVI_X_Turbine_Int, type = "state")
vcov_mat <- vcov(NDVI_X_Turbine_Int, type = "state")

# NDVI effect OUTSIDE wind farm (reference = 0) 
beta_outside <- coef_est["psi(NDVI_0_1km)"]

se_outside <- sqrt(
  vcov_mat["psi(NDVI_0_1km)", "psi(NDVI_0_1km)"]
)

# NDVI effect INSIDE wind farm (reference + interaction)
beta_inside <- coef_est["psi(NDVI_0_1km)"] +
  coef_est["psi(as.factor(turbine_interior)1:NDVI_0_1km)"]

var_inside <-
  vcov_mat["psi(NDVI_0_1km)", "psi(NDVI_0_1km)"] +
  vcov_mat["psi(as.factor(turbine_interior)1:NDVI_0_1km)",
           "psi(as.factor(turbine_interior)1:NDVI_0_1km)"] +
  2 * vcov_mat["psi(NDVI_0_1km)",
               "psi(as.factor(turbine_interior)1:NDVI_0_1km)"]
...
model_list <- list(
  ...
  "Turbine interior + NDVI" = Habitat_Add_Turbine_Int,
  "Turbine interior x biotic community" = Bio_Com_X_Turbine_Int,
  "Turbine interior x NDVI" = NDVI_X_Turbine_Int
)
...
final_table <- occu_model_selection(
  ...
  file_name = paste0(homewd, "outputs/cottontail_turbine_interior.csv")
)

#### Turbine Visibility Models ####

Turbine_Vis_Occu <- occu(  ~  veg_cover_cam_under_1m + 
                              as.factor(biotic_com_2)        
                           ~  X50cm_turbine_vis, occu.syau)

Habitat_Add_Turbine_Vis <- occu( ~  veg_cover_cam_under_1m + 
                                    as.factor(biotic_com_2)        
                                 ~  NDVI_0_1km + X50cm_turbine_vis, occu.syau)

Bio_Com_X_Turbine_Vis <- occu( ~  veg_cover_cam_under_1m + 
                                  as.factor(biotic_com_2)        
                               ~  X50cm_turbine_vis * as.factor(biotic_com_2),
                                  occu.syau)

Canopy_X_Turbine_Vis <- occu( ~  veg_cover_cam_under_1m + 
                                 as.factor(biotic_com_2)        
                              ~  X50cm_turbine_vis * canopy_cov, occu.syau)

NDVI_X_Turbine_Vis <- occu( ~  veg_cover_cam_under_1m + 
                               as.factor(biotic_com_2)        
                            ~  NDVI_0_1km * X50cm_turbine_vis, occu.syau)
...
model_list <- list(
  ...
  "Null" = Null,
  "Biotic community" = Bio_Com_Occu,
  "Canopy cover" = Canopy_Occu,
  "Turbine visibility" = Turbine_Vis_Occu,
  "Turbine visibility + NDVI" = Habitat_Add_Turbine_Vis,
  "Turbine visibility x biotic community" = Bio_Com_X_Turbine_Vis,
  "Turbine visibility x NDVI" = NDVI_X_Turbine_Vis,
...
wind_pairs <- list(
  ...
  "Turbine visibility x NDVI" = "NDVI",
...
final_table <- occu_model_selection(
  model_list = model_list,
  wind_pairs = wind_pairs,
  file_name = paste0(homewd, "outputs/cottontail_turbine_vis.csv")
)

#### Turbine Distance Models ####

Turbine_Dist_Occu <- occu( ~  veg_cover_cam_under_1m + 
                              as.factor(biotic_com_2)        
                           ~  turbine_dist, occu.syau)

Habitat_Add_Turbine_Dist <- occu( ~  veg_cover_cam_under_1m + 
                                     as.factor(biotic_com_2)        
                                  ~  NDVI_0_1km + turbine_dist, occu.syau)

Bio_Com_X_Turbine_Dist <- occu( ~  veg_cover_cam_under_1m + 
                                   as.factor(biotic_com_2)        
                                ~  turbine_dist * as.factor(biotic_com_2), 
                                   occu.syau)

NDVI_X_Turbine_Dist <- occu( ~  veg_cover_cam_under_1m + 
                                as.factor(biotic_com_2)        
                             ~  NDVI_0_1km * turbine_dist, occu.syau)
...
model_list <- list(
  ...
  "Turbine distance + NDVI" = Habitat_Add_Turbine_Dist,
  "Turbine distance x biotic community" = Bio_Com_X_Turbine_Dist,
  "Turbine distance x NDVI" = NDVI_X_Turbine_Dist
)
...
final_table <- occu_model_selection(
  model_list = model_list,
  wind_pairs = wind_pairs,
  file_name = paste0(homewd, "outputs/cottontail_turbine_dist.csv")
)

#### Access Road Distance Models ####

Turbine_Rd_Dist_Occu <- occu( ~  veg_cover_cam_under_1m + 
                                 as.factor(biotic_com_2)        
                              ~  turbine_rd_dist, occu.syau)

Habitat_Add_Turbine_Rd_Dist <- occu( ~  veg_cover_cam_under_1m + 
                                        as.factor(biotic_com_2)        
                                     ~  NDVI_0_1km + turbine_rd_dist, occu.syau)

Bio_Com_X_Turbine_Rd_Dist <- occu( ~  veg_cover_cam_under_1m + 
                                      as.factor(biotic_com_2)        
                                   ~  turbine_rd_dist * as.factor(biotic_com_2), 
                                      occu.syau)

NDVI_X_Turbine_Rd_Dist <- occu( ~  veg_cover_cam_under_1m + 
                                   as.factor(biotic_com_2)        
                                ~  NDVI_0_1km * turbine_rd_dist, occu.syau)
...
model_list <- list(
  ...
  "Turbine road distance + NDVI" = Habitat_Add_Turbine_Rd_Dist,
  "Turbine road distance x biotic community" = Bio_Com_X_Turbine_Rd_Dist,
  "Turbine road distance x NDVI" = NDVI_X_Turbine_Rd_Dist
)
...
wind_pairs <- list(
  ...
  "Turbine road distance x NDVI" = "NDVI",
...
final_table <- occu_model_selection(
  model_list = model_list,
  wind_pairs = wind_pairs,
  file_name = paste0(homewd, "outputs/cottontail_turbine_rd_dist.csv")
)

#### Access Road Density Models ####

Turbine_Rd_Dense_Occu <- occu( ~  veg_cover_cam_under_1m + 
                                  as.factor(biotic_com_2)        
                               ~  turbine_rd_density_0_1km, occu.syau)

Habitat_Add_Turbine_Rd_Dense <- occu( ~  veg_cover_cam_under_1m + 
                                         as.factor(biotic_com_2)        
                                      ~  NDVI_0_1km + turbine_rd_density_0_1km, 
                                         occu.syau)

Bio_Com_X_Turbine_Rd_Dense <- occu( ~  veg_cover_cam_under_1m + 
                                       as.factor(biotic_com_2)        
                                    ~  turbine_rd_density_0_1km *
                                       as.factor(biotic_com_2), occu.syau,
                                       starts = c(0,0,0,-3,-2,0,0))
#large SE for biotic comm and interaction

NDVI_X_Turbine_Rd_Dense <- occu( ~  veg_cover_cam_under_1m +
                                    as.factor(biotic_com_2)        
                                 ~  NDVI_0_1km * turbine_rd_density_0_1km, 
                                    occu.syau)
...
beta_grassland <- coef_est["psi(turbine_rd_density_0_1km)"]
se_grassland <- sqrt(vcov_mat["psi(turbine_rd_density_0_1km)", 
                              "psi(turbine_rd_density_0_1km)"])

beta_woodland <- coef_est["psi(turbine_rd_density_0_1km)"] + 
  coef_est["psi(turbine_rd_density_0_1km:as.factor(biotic_com_2)2)"]

var_woodland <- 
  vcov_mat["psi(turbine_rd_density_0_1km)", "psi(turbine_rd_density_0_1km)"] +
  vcov_mat["psi(turbine_rd_density_0_1km:as.factor(biotic_com_2)2)",
           "psi(turbine_rd_density_0_1km:as.factor(biotic_com_2)2)"] +
  2 * vcov_mat["psi(turbine_rd_density_0_1km)", 
               "psi(turbine_rd_density_0_1km:as.factor(biotic_com_2)2)"]
se_woodland <- sqrt(var_woodland)
...
model_list <- list(
  ...
  "Turbine road density + NDVI" = Habitat_Add_Turbine_Rd_Dense,
  "Turbine road density x biotic community" = Bio_Com_X_Turbine_Rd_Dense,
  "Turbine road density x NDVI" = NDVI_X_Turbine_Rd_Dense
)
...
wind_pairs <- list(
  ...
  "Turbine road density x NDVI" = "NDVI",
...
final_table <- occu_model_selection(
  model_list = model_list,
  wind_pairs = wind_pairs,
  file_name = paste0(homewd, "outputs/cottontail_turbine_rd_density.csv")
)
